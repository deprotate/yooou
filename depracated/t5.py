...
def add_heuristic_features(df, train_history_df, book_meta):
    ...
    # User's favorite authors (count)
    # Это может быть медленно, сделаем проще:
    # Средний рейтинг юзера по автору
    ...
    # Чтобы смержить user_author_stats, нам нужен author_id в df
...
def clean_data_for_catboost(df, cat_cols):
    ...
    """
    # 1. Находим текстовые колонки, которых нет в списке cat_cols
    ...
    # 2. Гарантируем, что категории это строки (CatBoost не любит смешанные типы)
    for c in cat_cols:
        if c in df.columns:
            df[c] = df[c].astype(str)
            # Заменяем 'nan' строку на пустую или спец токен, если нужно, но astype(str) обычно хватает
...
def main():
    ...
    # 2. SVD
    svd_data = train_svd_model(train_df)

    # 3. BERT
    bert_embs = compute_bert_embeddings(desc_df)

    # 4. Validation Split
    print("Splitting train/val by time...")
    ...
    # 5. Build Features
    train_feat, cat_cols = build_features(train_part_neg, u_meta, b_meta, desc_df, svd_data, bert_embs, train_part)
    val_feat, _ = build_features(val_part_neg, u_meta, b_meta, desc_df, svd_data, bert_embs, train_part)

    # === ВАЖНОЕ ИСПРАВЛЕНИЕ: Сортировка по ГРУППАМ (USER_ID) ===
    # CatBoostRanker требует, чтобы все записи одного юзера шли подряд!
    print("Sorting data by User ID for YetiRank...")
    ...
    # Чистка типов
    print("Cleaning data types for CatBoost...")
    ...
    print(f"Final feature list len: {len(X_tr.columns)}")
    ...
    del train_part, val_part, train_feat, val_feat, X_tr, y_tr, X_val, y_val, train_pool, val_pool, model
    gc.collect()

    # 6. REFIT (Full Data)
    print("\n=== REFIT PHASE ===")
    train_full_neg = generate_negatives(train_df, all_books)
    train_full_feat, _ = build_features(train_full_neg, u_meta, b_meta, desc_df, svd_data, bert_embs, train_df)

    # Сортировка обязательна и здесь!
    ...
    final_model.save_model(Config.MODEL_DIR / "catboost_refit.cbm")

    # 7. Inference
    print("Inference...")
    cand_exp = expand_candidates(cand_df)
    cand_feat, _ = build_features(cand_exp, u_meta, b_meta, desc_df, svd_data, bert_embs, train_df)
    ...
    final_sub.to_csv(Config.SUBMISSION_DIR / "submission_v3_refit.csv", index=False)
    print("Done!")
