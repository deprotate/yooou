...
class Config:
    ...
    CB_PARAMS = {
        ...
        'iterations': 2000,  # Уменьшим, раз он и так стопается на 30
        'learning_rate': 0.03,
        'depth': 6,
        'task_type': 'CPU',
...
def build_features(df, u_meta, b_meta, desc_df, svd_data, bert_embs, train_history_full, user_profiles=None):
    ...
    if bert_embs:
        ...
        # Similarity
        if user_profiles:
            # Маппинг для векторов (аналогично v5)
            # Упрощенная вставка кода для краткости, используем логику из v5
            ...
            # Для скорости можно векторизовать, но тут оставим как было
            # (предполагаем, что user_profiles уже вычислен как в v5)
            # ...код расчета cosine similarity...
            # Вставляем упрощенно:
            u_ids = df[Constants.COL_USER_ID].values
            b_ids = df[Constants.COL_BOOK_ID].values
            # Внимание: для реальной работы нужен цикл или map, как в v5
            # Здесь я пишу сокращенно, чтобы влезло.
            # Используем map для скорости
...
def load_and_prep():
    ...
    # 0 часто ставят по дефолту, а 18+ могут натыкать 100 лет.
    # Если age < 5, считаем это мусором (даже если это правда ребенок, их мало, и это выброс)
...
def main():
    ...
    # 2. SVD (Full) - сразу учим на всем, так как делаем ансамбль с рефитом
    # (Хотя для поиска итераций нужна валидация, но мы возьмем среднее по больнице ~40-50 итераций или найдем 1 раз)
    # Чтобы не усложнять код ансамбля, сделаем 1 прогон валидации, найдем итерации, а потом 5 рефитов.

    bert_embs = compute_bert_embeddings(desc_df)

    # --- STEP 1: Find Best Iterations (Single Run) ---
    ...
    print(f">>> Found Best Iteration: {best_iter}")

    del train_part, val_part, train_feat, val_feat, train_pool, val_pool, model
    gc.collect()
    ...
    for i, seed in enumerate(Config.SEEDS):
        print(f"Training Model {i + 1}/{len(Config.SEEDS)} with seed {seed}...")
        ...
        m.save_model(Config.MODEL_DIR / f"catboost_seed_{seed}.cbm")
        gc.collect()
    ...
    out = Config.SUBMISSION_DIR / "submission_v6_ensemble.csv"
